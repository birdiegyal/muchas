...
function mapReducer(state, action) {
  switch (action.type) {
    case "flyTo":
      ...
      return {
        ...state,
        flyTo: [action.state.flyToLng, action.state.flyToLat],
      };
    default:
      return state;
  }
...
*/
    ...
    if (initState.flyTo.length > 0) {
      // console.log("s")
      new mapboxgl.Marker().setLngLat(initState.flyTo).addTo(map.current);
      map.current.flyTo({
    ...
    const locator = navigator.geolocation.watchPosition(
      success,
      error,
      options
    );
